...
app.post("/api/auth/register", async (req, res) => {
  ...
  const existing = (await pool.query("SELECT id FROM users WHERE email = ?", [email])).rows[0];
  if (existing) {
...
app.post("/api/auth/oauth/config", (req, res) => {
  ...
  try {
    ...
    console.error("Failed to write to .env in server-pg:", e.message);
  }
...
app.post("/api/auth/oauth/callback", async (req, res) => {
  try {
    ...
    // Tra cứu và lưu trữ trong PostgreSQL Database
    ...
    await pool.query(`
      INSERT INTO user_activity (user_id, activity_type, details, xp_earned)
      VALUES (?, 'auth', ?, 15)
    `, [user.id, `Đăng nhập thành công qua ${provider.toUpperCase()} OAuth 2.0 (Authorization Code Flow)`]);
    ...
    console.error("OAuth Callback Error in PG:", err);
...
app.get("/api/auth/me", async (req, res) => {
  ...
  if (userId) {
    ...
    try {
      ...
      if (email) {
        user = (await pool.query("SELECT * FROM users WHERE email = ?", [email])).rows[0];
      }
    } catch (e) {}
  }
  ...
  if (!user) {
    ...
    user = (await pool.query("SELECT * FROM users WHERE role = 'admin' LIMIT 1", [])).rows[0];
  }
...
app.put("/api/auth/profile", async (req, res) => {
  ...
  await pool.query(`
    ...
    WHERE id = ?
  `, [updatedName, updatedPro, updatedStreak, updatedDiamonds, id]);

  const updated = (await pool.query("SELECT * FROM users WHERE id = ?", [id])).rows[0];
...
app.post("/api/shop/purchase", async (req, res) => {
  ...
  const user = (await pool.query("SELECT * FROM users WHERE id = ?", [user_id])).rows[0];
  if (!user) return res.status(404).json({ error: "User not found" });
...
app.get("/api/notes", async (req, res) => {
  ...
  let notes = (await pool.query(query, [...params])).rows;
  if (notes.length === 0 && Number(userId) !== 1) {
    ...
    notes = (await pool.query(fallbackQuery, [...fallbackParams])).rows;
  }
  res.json(notes);
});
...
app.put("/api/notes/:id", async (req, res) => {
  ...
  const note = (await pool.query("SELECT * FROM notes WHERE id = ?", [noteId])).rows[0];
...
app.put("/api/admin/users/:id/pro", async (req, res) => {
  const userId = req.params.id;
  const user = (await pool.query("SELECT is_pro FROM users WHERE id = ?", [userId])).rows[0];
  if (!user) return res.status(404).json({ error: "User not found" });

  const newPro = user.is_pro === 1 ? 0 : 1;
  await pool.query("UPDATE users SET is_pro = ? WHERE id = ?", [newPro, userId]);
