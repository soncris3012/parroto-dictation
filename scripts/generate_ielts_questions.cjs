...
// Read existing file text and parse JSON or extract
const filePath = path.join(__dirname, "../src/data/ieltsExamsData.js");
const raw = fs.readFileSync(filePath, "utf8");
const jsonStr = raw.replace(/^export const ieltsExamsData = /, "").replace(/;[\s\n]*$/, "");
const ieltsExamsData = JSON.parse(jsonStr);
...
ieltsExamsData.forEach((book, bookIdx) => {
  book.exams.forEach((exam, examIdx) => {
    ...
    totalQGenerated += allQ.length;
  });
});

console.log(`Generated ${totalQGenerated} full IELTS questions across ${ieltsExamsData.length} books!`);

const outputContent = "export const ieltsExamsData = " + JSON.stringify(ieltsExamsData, null, 2) + ";\n";
fs.writeFileSync(filePath, outputContent, "utf8");
console.log("Successfully updated ieltsExamsData.js!");
